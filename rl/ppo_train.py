...
def main() -> None:
    ...
    # Use MlpPolicy for pattern generation (MultiDiscrete action space)
    model = PPO(
        "MlpPolicy",  # MlpPolicy supports MultiDiscrete action spaces
        vec_env,
        verbose=1,
        learning_rate=3e-4,
        n_steps=64,  # Reduced for single-step environment (each step = one complete GDiet run)
        batch_size=32,  # Reduced proportionally
        n_epochs=4,  # Reduced for faster training
        gamma=0.99,
        gae_lambda=0.95,
        clip_range=0.2,
        ent_coef=0.1,  # Increased for pattern generation (more exploration needed)
        vf_coef=0.5,
